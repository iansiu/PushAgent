...
Для поддержки приложения QiuyuRemote, обратной связи, сообщений об ошибках,
запросов функций и вопросов по PushAgent создайте issue здесь:
...
Если Push Relay принимает событие, но его не получает ни одно устройство или получают только некоторые устройства, Agent оставляет событие в состоянии ожидания и повторяет отправку в следующих циклах. Устройства, которые уже получили событие, исключаются из повторной отправки, а недавние дубликаты подавляются, поэтому восстановление доставки не создает повторные успешные уведомления.
...
| `monitor.serviceStatusNoticeEnabled` | Отправлять ли уведомления, когда отслеживаемая служба загрузки уходит offline или снова становится online. По умолчанию `true`; `false` отключает только уведомления о состоянии службы, сохраняя уведомления о задачах. |
